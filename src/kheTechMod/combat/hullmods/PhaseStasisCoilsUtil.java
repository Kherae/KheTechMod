...
//    public static float TIMEWARP_EFFECTIVENESS_MULT=0.5f;
	...
	public static boolean hasOtherPhaseStasis(String id, ShipAPI ship) {
		...
		for (String phaseStasisModel : PhaseStasisModels) {
			if ((!Objects.equals(phaseStasisModel, id)) && KheUtilities.shipHasHullmod(ship, phaseStasisModel)) {
				result = true;
			} else if (Objects.equals(phaseStasisModel, id)) {
				foundSelf = true;
			}
		}
	...
	public void handlePhase(ShipAPI ship, String myModel, boolean doTimeWarp, boolean clampTime, float advanceAmount, float timewarpEffectiveness) {
		...
		if (ship.isPhased()) {
			...
			if (isPlayer) {
				//KheTimeCleanup.registerTimeEffect(instancedModel);//moved to handleTimeFlow in KheUtilities
				if (doTimeWarp) {
					...
					if (cloak != null) {
						...
						if (clampTime) {
							buffer = Math.max(1f, buffer);
						}
						if (timewarpEffectiveness != 1f) {
							buffer = (buffer - 1f) * timewarpEffectiveness + 1f;
						}
	...
	public static void applyTimeMultiplier(ShipAPI ship, float timeMultNow, boolean isPlayer, String effectID, boolean timeWarp, boolean clampTime, float timewarpEffectiveness, float advanceAmount) {
		KheUtilities.handleTimeFlow(true, effectID, isPlayer, ship, 1f / timeMultNow);

		if (timeWarp) {
			if (clampTime) {
				timeMultNow = Math.max(1f, timeMultNow);
			}
			if (timewarpEffectiveness != 1f) {
				timeMultNow = (timeMultNow - 1f) * timewarpEffectiveness + 1f;
			}
	...
	public static void unApplyTimeMultiplier(ShipAPI ship, String effectID, boolean timeWarp) {
		KheUtilities.handleTimeFlow(false, effectID, false, ship, 1f);
		if (timeWarp) {
	...
	public static void tooltipHandler(
		...
		if (upkeepModifier != 1f) {
			upkeep = true;
		}
		if (fluxThresholdModifier > 0f) {
			threshold = true;
		}
		...
		if (upkeep) {
			...
			if (upkeepModifier > 1.0f) {
				upkeepHighlight = bad;
			} else {
				upkeepHighlight = good;
			}
		...
		if (timeWarp) {
			...
			if (clampTime) {
				buffer += ", but may not bring the ship below 1.0x timeflow.";
			} else {
				buffer += ".";
			}
