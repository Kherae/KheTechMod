...
public class VentHack extends BaseHullMod {
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
		if (index == 0) {
			return (NATIVE_VENT_RATE * BASE_VENT_MULTIPLIER) + "x";
		}
		if (index == 1) {
			return FLUX_CAPACITY_MULT + "x";
		}
		if (index == 2) {
			return KheUtilities.lazyKheGetMultString(SUPPLY_UPKEEP_PENALTY);
		}
		return "PIGEON";
	}
...
//			return "Cannot be installed on ships that natively have shields.";
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		tooltip.addPara(
				...
				KheUtilities.lazyKheGetMultString(FLUX_CAPACITY_MULT), KheUtilities.lazyKheGetMultString(0),
				KheUtilities.lazyKheGetMultString(SUPPLY_UPKEEP_PENALTY)
		);
	}
