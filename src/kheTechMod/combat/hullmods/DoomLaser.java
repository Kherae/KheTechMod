...
public class DoomLaser extends BaseHullMod {
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		tooltip.addPara("Beam damage: %s\nBeam turn speed: %s", opad, good,
				KheUtilities.lazyKheGetMultString(DMG_MULT),
				KheUtilities.lazyKheGetMultString(TURN_BOOST)
		);
		tooltip.addPara(
				"Beam range: %s\nBeam flux: cost %s\nALL weapon fire rate: %s", opad, bad,
				KheUtilities.lazyKheGetMultString(RANGE_PENALTY),
				KheUtilities.lazyKheGetMultString(COST_MULT),
				KheUtilities.lazyKheGetMultString(FIRE_PENALTY)
		);
	}
