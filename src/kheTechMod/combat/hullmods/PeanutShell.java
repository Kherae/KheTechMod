...
public class PeanutShell extends BaseHullMod {
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
		if (index == 0) {
			return KheUtilities.lazyKheGetMultString(BONUS_MULT);
		}
		if (index == 1) {
			return KheUtilities.lazyKheGetMultString(PENALTY_MULT);
		}
		if (index == 2) {
			return KheUtilities.lazyKheGetMultString(LOGISTICS_MULT);
		}
		return "PIGEON";
	}
...
//			return "Cannot be installed on ships that natively have shields.";
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		tooltip.addPara(
				...
				KheUtilities.lazyKheGetMultString(PENALTY_MULT), KheUtilities.lazyKheGetMultString(PENALTY_MULT),
				KheUtilities.lazyKheGetMultString(PENALTY_MULT),
